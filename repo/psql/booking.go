...
const bookingSelect = `SELECT b.id, COALESCE(b.description, ''), b.delete_id, lower(b.during), upper(b.during), b.booking_type_id, COALESCE(b.availability_id, 0),
	...
	b.paid, COALESCE(b.note, ''), b.confirmed
...
func (r *Repo) CreateBooking(ctx context.Context, b *deiz.Booking) error {
	const query = `INSERT INTO clinician_booking(address, price, description, booking_type_id, availability_id, clinician_person_id, patient_id, during, paid, note, confirmed)
	VALUES(NULLIF($1, ''), $2, NULLIF($3, ''), $4, NULLIF($5, 0), $6, NULLIF($7, 0), tsrange($8, $9, '()'), $10, NULLIF($11, ''), $12)
	RETURNING id, delete_id`
	row := r.conn.QueryRow(ctx, query, b.Address, b.Price, b.Description, b.BookingType, b.AvailabilityType, b.Clinician.ID, b.Patient.ID, b.Start, b.End, b.Paid, b.Note, b.Confirmed)
...
func (r *Repo) UpdateBooking(ctx context.Context, b *deiz.Booking) error {
	...
	SET address = NULLIF($1, ''), price = NULLIF($2, 0), description = NULLIF($3, ''), booking_type_id = $4, clinician_person_id = $5, patient_id = $6,
	during = tsrange($7, $8, '()'), paid = $9, note = NULLIF($10, ''), confirmed = $11, availability_id = $5 WHERE id = $12`
